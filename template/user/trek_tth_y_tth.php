...
$data = $wpdb->get_results('SELECT * FROM ' . $table_prefix . 'tth_why where tth_why_status=0 order by id desc');
        ...
        <div class="col-md-10" style="min-height: 450px;background-color: #C0C7D3;">
            ...
            <div class="col-md-3 card gallarybox2 set-padding" style="margin-left: 26px;">
                <div class="form-group"><label for="trek_Transportation">Gallery Images
                        * <br><i>(First three images will be displayed)</i></label>
            ...
            <div class="col-md-8 set-padding">
                <label for="tth_why_caption"><b>Image caption</b></label><textarea rows="4" id="tth_why_caption" placeholder="caption" style="width: 100%"
                                                               ><?php echo stripslashes($data[0]->tth_why_caption); ?></textarea>
                <button type="button" id="<?php echo $data[0]->id; ?>" class="btn btn-primary" onclick="savethedata(this.id)">Save</button>
            </div>
...
<script>
    function savethedata(id)
    {
        ...
        else
        {
            ...
            data.append('id', id);
            data.append('action', 'whytthsetup');
            jQuery.ajax({
                type: "post",
                cache: false,
                contentType: false,
